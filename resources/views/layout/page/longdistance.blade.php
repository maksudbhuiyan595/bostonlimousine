...
{{-- ১. ডাইনামিক বা ফলব্যাক মেটা ডাটা (Long Distance Context) --}}
@section('title', $main_page->title ?? "Long Distance Taxi Service Boston | Ride to CT, RI, NH, NY, VT")
@section('meta_description', $main_page->meta_description ?? "Safe and affordable long distance taxi rides from Boston to New York, Connecticut, Rhode Island, New Hampshire, and beyond. 24/7 premium fleet with child seats. Call 617-230-6362.")
        ...
        @foreach($main_page->content_blocks as $block)
            ...
            @if($block['type'] === 'cta' || isset($block['data']['button_text']))
                @php
                    ...
                    $btnText  = $block['data']['button_text'] ?? $block['button_text'] ?? 'Book Long Distance Ride';
                                ...
                                @foreach($images as $image)
                                    <div class="col-md-4 col-sm-6">
                                        <div class="card h-100 shadow-sm">
                                            <img src="{{ asset('storage/' . $image) }}" class="card-img-top img-fluid rounded" alt="Long Distance Fleet">
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <div class="col-md-6">
                                    <img src="{{ asset('storage/' . $images) }}" class="img-fluid rounded shadow" alt="Long Distance Fleet">
                                </div>
                            @endif
                        </div>
                    </div>
