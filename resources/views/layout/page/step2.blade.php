...
<div class="booking-wrapper animate__animated animate__fadeIn">
    ...
    <div class="breadcrumb-nav">
        <i class="fas fa-home" style="color: #B9924B;"></i> Home &raquo; Reservation &raquo; <span>Choose vehicle (Step 2 of 4)</span>
    </div>
    ...
    <form id="bookingForm" action="{{ route('step3') }}" method="GET">
        ...
        <input type="hidden" name="fare[estimatedFare]" id="inp_base" value="{{ $defaultVehicle['estimated_fare'] ?? 0 }}">
        <input type="hidden" name="fare[gratuity]" id="inp_grat" value="{{ $defaultVehicle['gratuity_fee'] ?? 0 }}">
        <input type="hidden" name="fare[surcharge_fee]" id="inp_sur" value="{{ $defaultVehicle['surcharge_fee'] ?? 0 }}">
        <input type="hidden" name="fare[extra_luggage_fee]" id="inp_elug" value="{{ $defaultVehicle['extra_luggage_fee'] ?? 0 }}">
        ...
        <div class="row">
            ...
            <div class="col-lg-4">
                ...
                <div class="pay-box-container">
                    ...
                    <div class="pay-col">
                        ...
                        <span class="pay-amt text-red">$<span id="disp_cash">{{ isset($defaultVehicle['pay_cash']) ? number_format($defaultVehicle['pay_cash'], 2) : '0.00' }}</span></span>
                        <div class="pay-note">Pay $1 Reservation Fee</div>
                    </div>
                    <div class="pay-col">
                        <span class="pay-lbl"><i class="fas fa-credit-card"></i> Pay Card</span>
                        <span class="pay-amt">$<span id="disp_card">{{ isset($defaultVehicle['total_fare']) ? number_format($defaultVehicle['total_fare'], 2) : '0.00' }}</span></span>
                        <div class="pay-note">Full Online Payment</div>
                    </div>
                </div>
                <div class="bottom-note">
                    <i class="fas fa-shield-alt me-1" style="color: #B9924B;"></i> <strong>Secure Booking:</strong> Pay only $1.00 now to confirm your reservation. Balance is payable after service.
                </div>
            </div>
                ...
                <div class="pricing-card">
                    ...
                    <table class="pricing-table">
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td>$<span id="tbl_base">{{ isset($defaultVehicle['estimated_fare']) ? number_format($defaultVehicle['estimated_fare'], 2) : '0.00' }}</span></td>
                        </tr>
                        <tr>
                            <td>Gratuity (20%)</td>
                            <td>:</td>
                            <td>$<span id="tbl_grat">{{ isset($defaultVehicle['gratuity_fee']) ? number_format($defaultVehicle['gratuity_fee'], 2) : '0.00' }}</span></td>
                        </tr>
                        ...
                        <tr id="row_sur" style="{{ (isset($defaultVehicle['surcharge_fee']) && $defaultVehicle['surcharge_fee'] > 0) ? '' : 'display:none;' }}">
                            <td>Surcharge / Night</td>
                            <td>:</td>
                            <td>$<span id="tbl_sur">{{ isset($defaultVehicle['surcharge_fee']) ? number_format($defaultVehicle['surcharge_fee'], 2) : '0.00' }}</span></td>
                        </tr>

                        <tr class="border-top">
                            <td class="pt-3 fw-bold text-dark">Total Payable</td>
                            <td class="pt-3">:</td>
                            <td class="pt-3 fw-bold text-dark fs-5" style="color: #B9924B !important;">$<span id="tbl_total">{{ isset($defaultVehicle['total_fare']) ? number_format($defaultVehicle['total_fare'], 2) : '0.00' }}</span></td>
                        </tr>
                    </table>

                    <div id="box_elug" class="extra-lug-box" style="{{ (isset($defaultVehicle['extra_luggage_fee']) && $defaultVehicle['extra_luggage_fee'] > 0) ? '' : 'display:none;' }}">
                        <span class="fw-bold"><i class="fas fa-luggage-cart me-2"></i>Extra Luggage Fee</span>
                        <span class="fw-bold fs-5">$<span id="disp_elug">{{ isset($defaultVehicle['extra_luggage_fee']) ? number_format($defaultVehicle['extra_luggage_fee'], 2) : '0.00' }}</span></span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="btn-pay-note text-muted text-center mb-3">Pay only $1 & confirm your reservation. Balance is payable after service</div>
                ...
                <div class="summary-box">
                    ...
                    <table class="sum-table">
                        <tr><td>Service</td><td>:</td><td class="sum-val">{{ $trip_type == 'fromAirport' ? '✈️ From Airport' : ($trip_type == 'toAirport' ? '🛫 To Airport' : '🚗 Door-to-Door') }}</td></tr>
                        <tr><td>Date</td><td>:</td><td class="sum-val">{{ $formattedDate ?? ($request['date'] ?? 'N/A') }}</td></tr>
                        <tr><td>Time</td><td>:</td><td class="sum-val">{{ $formattedTime ?? ($request['time'] ?? 'N/A') }}</td></tr>
                        <tr><td>Pick up</td><td>:</td><td class="sum-val">{{ Str::limit($pickup ?? 'N/A', 28) }}</td></tr>
                        <tr><td>Drop off</td><td>:</td><td class="sum-val">{{ Str::limit($dropoff ?? 'N/A', 28) }}</td></tr>
                        <tr><td>Passengers</td><td>:</td><td class="sum-val">{{ $reqPassengers ?? 0 }} ({{ $request['adults'] ?? 0 }} Adults + {{ $child_seat ?? 0 }} Children)</td></tr>
                        <tr><td>Luggage</td><td>:</td><td class="sum-val">{{ $request['luggage'] ?? 0 }} bags</td></tr>
                        @if(($request['infant_seat'] ?? 0) > 0)<tr><td>Infant Seat</td><td>:</td><td class="sum-val">{{ $request['infant_seat'] }}</td></tr>@endif
                        @if(($request['booster_seat'] ?? 0) > 0)<tr><td>Booster Seat</td><td>:</td><td class="sum-val">{{ $request['booster_seat'] }}</td></tr>@endif
                        <tr><td colspan="3"><hr style="border-color:#E5E7EB; margin:15px 0;"></td></tr>
                        <tr><td colspan="3" style="font-weight:700; color:#B9924B; text-transform:uppercase; font-size:0.8rem;"><i class="fas fa-car me-1"></i> Vehicle Info</td></tr>
                        <tr><td>Selected</td><td>:</td><td class="sum-val"><span id="sum_vname">{{ $defaultVehicle['name'] ?? 'N/A' }}</span></td></tr>
                        <tr><td>Capacity</td><td>:</td><td class="sum-val"><span id="sum_vpax">{{ $defaultVehicle['capacity_passenger'] ?? 0 }}</span> Pax, <span id="sum_vlug">{{ $defaultVehicle['capacity_luggage'] ?? 0 }}</span> Bags</td></tr>
                        <tr><td colspan="3"><hr style="border-color:#E5E7EB; margin:15px 0;"></td></tr>
                        <tr><td class="fw-bold fs-6 text-dark">ESTIMATED TOTAL</td><td class="fw-bold fs-6">:</td><td class="fw-bold fs-5" style="color:#B9924B;">$<span id="sum_total">{{ isset($defaultVehicle['total_fare']) ? number_format($defaultVehicle['total_fare'], 2) : '0.00' }}</span></td></tr>
                    </table>
                </div>
            </div>
        </div>
    ...
    <div id="vehicle_list_container">
        @if(isset($vehicleOptions) && count($vehicleOptions) > 0)
            @foreach($vehicleOptions as $option)
                ...
                @endphp
                <div class="veh-row {{ $isCurrent ? 'd-none-custom' : '' }}"
                    id="vrow_{{ $option['vehicle_id'] ?? '' }}"
                    onclick="updateVehicle({{ json_encode($option) }})">

                    <div class="vr-img-box">
                        <img src="{{ asset($option['image'] ?? 'images/home1.jpeg') }}" class="vr-img" alt="{{ $option['name'] ?? 'Vehicle' }}">
                    </div>

                    <div class="vr-info flex-grow-1">
                        <h5>{{ $option['name'] ?? 'Vehicle' }} <i class="fas fa-crown" style="color: #B9924B; font-size: 0.9rem;"></i></h5>
                        <div class="vr-meta">
                        ...
                        <div class="text-muted" style="font-size:0.8rem">
                            Base Fare: ${{ number_format($option['estimated_fare'] ?? 0, 2) }} &bull; Gratuity: ${{ number_format($option['gratuity_fee'] ?? 0, 2) }}
                        </div>
                    </div>
